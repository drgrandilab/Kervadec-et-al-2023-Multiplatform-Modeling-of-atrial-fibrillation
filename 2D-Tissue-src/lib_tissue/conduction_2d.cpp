		...
		for (i = 1; i < nx + 1; ++i) {
			if (atrium[j][i] > 0) {
				...
				if ( (atrium[j][i + 1] > 0) && (atrium[j][i - 1] > 0) &&
					...
					// std::cout << dc[j][i][0] << std::endl;
					// std::cout << dc[j][i][1] << std::endl;
					// std::cout << dc[j][i][2] << std::endl;
					// std::cout << dc[j][i][3] << std::endl;
					...
					/*double factor = 1.0 / 6.0 * (1 / dx) * (1 / dx) * dc[j][i][0];
					lap[j][i][0] += -8.0 * factor;
					lap[j][i][1] +=  4.0 * factor;
					lap[j][i][2] +=  4.0 * factor;
					lap[j][i][3] += -2.0 * factor;
					lap[j][i][4] += -2.0 * factor;
					lap[j][i][5] +=  1.0 * factor;
					lap[j][i][6] +=  1.0 * factor;
					...
					// dvdy2
					factor = 1.0 / 6.0 * (1 / dy) * (1 / dy) * dc[j][i][3];
					lap[j][i][0] += -8.0 * factor;
					lap[j][i][1] += -2.0 * factor;
					lap[j][i][2] += -2.0 * factor;
					lap[j][i][3] +=  4.0 * factor;
					lap[j][i][4] +=  4.0 * factor;
					lap[j][i][5] +=  1.0 * factor;
					lap[j][i][6] +=  1.0 * factor;
					lap[j][i][7] +=  1.0 * factor;
					lap[j][i][8] +=  1.0 * factor;*/
					...
					// 9 point stencil
					// https://pdf.sciencedirectassets.com/271503/1-s2.0-S0898122199X00010/1-s2.0-0898122175900358/main.pdf?X-Amz-Security-Token=IQoJb3JpZ2luX2VjEP7%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FwEaCXVzLWVhc3QtMSJHMEUCIEmyTcwngEgoLRbNPN3ZpsdDn6C6t8oJDI9Mdaaevu2oAiEA68CUATWYOIWGLfM0UQL%2B60JK6CWmqVXanlZtXkD%2BFxkqgwQIt%2F%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FARAEGgwwNTkwMDM1NDY4NjUiDEAwnb8KQsQUJoDS5SrXA93oKe0Kpi%2B%2FG%2B5rPqIsFATJ3uR6vF%2FQTPL61bAmk5sqWFBaPCbFgagIuYtBZFZRZdDQLybliPUUSO7l0OIp3zy1Kff5qIJK8X%2BCVYDSkCHJdFLY1NDFhktOP5ltd0Bcj0BvCFeAP4v%2FbeeI6RAyVMaKehHTAJx0MbzVbXBVldRYl%2Fq%2FTujbgwdgkGVAR28PnPSfRjQe9YCekv830jbKhy2drWO7GD7w2VM9%2BHmMhZ2MTMSc8Jr4L%2FN%2FBYxnLLMBvAim0w7Isl1As9S6pEvOd8Avkq3MRgjerOEDeD%2BzG0l3QxNa%2BZC5XRi6ozG%2BQLZ9EEb4gs4GmVjURlqwuyB0BjavLa1k%2F4fXgArRBJ2PaKSvM1pXeLtIV4PmJqaeEVpCtDhhieN1cOQtipOAXp8k%2B1KhfahwjanCb86LWIU5MRB37eeKnvnAvTKb%2Bh9i%2BoPJKhboRX9AXoJNSRCvi4XPuk55Nt34lxOwHZvatVi6Aquh%2FSPKg0Q4u2A4nvkFoCQaiEFqzqpCzUMoPJGztADeb7SRRN8d8enESCkQJevJGYyQcKDQn%2FFD4ByaqXA8R35iXs0IRuGRuzw7poeNC1Py0Bky509B541O36%2BPzZ89qkvTwbqYK8p%2FRjDQ9IuGBjqlAaqKrVuZjHTzItxXQQP02g0%2BFisNTwSVkN7Rb5rHBh4dlBpBVHbhzSp5IBWcsxZ3vSxaiKhg3N8ndD6O%2F8FinH%2BSNNgVJJMP0FYqWG%2FpvtEeNBJk6DygcM6VOn2FNZlu3tpqu5%2BJLukVEzB4WJj9mbNj3t8mueqzTsBatP1djHBAWdKIUGhSglDIrarvxSta2JaaOCag1K7VeU52giIyrRBuQxixaA%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20210611T062617Z&X-Amz-SignedHeaders=host&X-Amz-Expires=300&X-Amz-Credential=ASIAQ3PHCVTY45WX7GOL%2F20210611%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=6701f6ae32db53e422186ab3577f4bdf4c1fe6c659779995be4e4dd2268734bf&hash=3132388fa1e680d821f7a7f99762c53db7d4cc5eb27ffa5351412be7220c9bc9&host=68042c943591013ac2b2430a89b270f6af2c76d8dfd086a07176afe7c76c2c61&pii=0898122175900358&tid=spdf-e1d43b04-2bf9-4b19-9eb8-0cec98cb3271&sid=75d90f7f72d7f8430b089d58711b4076dfe1gxrqa&type=client
					// https://docs.lib.purdue.edu/cgi/viewcontent.cgi?article=1928&context=cstech
					double factor = 1.0 / 6.0 * (1 / dx) * (1 / dx) * dc[j][i][0];
					lap[j][i][0] += -10.0 * factor;
					lap[j][i][1] +=  5.0 * factor;
					lap[j][i][2] +=  5.0 * factor;
					lap[j][i][3] += -1.0 * factor;
					lap[j][i][4] += -1.0 * factor;
					lap[j][i][5] +=  0.5 * factor;
					lap[j][i][6] +=  0.5 * factor;
					lap[j][i][7] +=  0.5 * factor;
					lap[j][i][8] +=  0.5 * factor;
					...
					lap[j][i][3] +=  5.0 * factor;
					lap[j][i][4] +=  5.0 * factor;
					lap[j][i][5] +=  0.5 * factor;
					lap[j][i][6] +=  0.5 * factor;
					lap[j][i][7] +=  0.5 * factor;
					lap[j][i][8] +=  0.5 * factor;
